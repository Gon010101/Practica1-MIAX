...
def clean_nans(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if rows_removed > 0:
        print(f"⚠️ Se eliminaron {rows_removed} filas con valores faltantes irrecuperables")
...
def calculate_log_returns(df: pd.DataFrame, price_column: str = 'Close') -> pd.DataFrame:
    ...
    if price_column not in df_with_returns.columns:
        raise ValueError(f"Columna '{price_column}' no encontrada en el DataFrame")
    
    # Calcular log returns
    df_with_returns['Log_Returns'] = np.log(df_with_returns[price_column] / df_with_returns[price_column].shift(1))
    
    # El primer valor será NaN (no hay valor anterior)
    # Lo eliminamos o lo dejamos para mantener consistencia con las fechas
...
def validate_dataframe(df: pd.DataFrame) -> Tuple[bool, list]:
    ...
    # Verificar que tenga al menos una columna de precios
    price_columns = ['Open', 'High', 'Low', 'Close']
    has_price = any(col in df.columns for col in price_columns)
    if not has_price:
        errors.append("No se encontró ninguna columna de precios (Open, High, Low, Close)")
...
def preprocess_financial_data(df: pd.DataFrame) -> Tuple[pd.DataFrame, list]:
    ...
    # 4. Calcular log returns
    df_processed = calculate_log_returns(df_processed)
